<template>
  <div class="space-y-6">
    ...
    <!-- ============================================================================== -->
    <!-- 👑 ROLE 0: SUPERADMIN DASHBOARD (Platform Master & Multi-Tenant Control) -->
    <!-- ============================================================================== -->
    <template v-if="effectiveRole === 'superadmin'">
      <!-- Platform Master KPI Cards -->
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 flex items-center justify-between">
          <div>
            ...
            <h3 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">{{ workspaceStore.workspaces.length || 2 }}</h3>
            <p class="mt-1 text-xs text-purple-600 dark:text-purple-400 font-medium">Tenant Aktif Multi-Tenant</p>
          </div>
          <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-purple-50 text-purple-600 dark:bg-purple-500/15">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 flex items-center justify-between">
          <div>
            ...
            <h3 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">{{ workspaceStore.members.length || 4 }}</h3>
            <p class="mt-1 text-xs text-success-600 font-medium">Pengguna Terdaftar</p>
          </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 flex items-center justify-between">
          <div>
            <p class="text-xs font-semibold uppercase tracking-wider text-gray-500">Proyek & Tugas Global</p>
            <h3 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">{{ projectStore.projects.length }} / {{ taskStore.tasks.length }}</h3>
            <p class="mt-1 text-xs text-blue-500 font-medium">{{ taskStore.completedTasks.length }} Tugas Selesai</p>
          </div>
        ...
        <!-- Platform Storage & RLS -->
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 flex items-center justify-between">
          <div>
            <p class="text-xs font-semibold uppercase tracking-wider text-gray-500">Kesehatan Database</p>
            <h3 class="mt-2 text-2xl font-bold text-success-600">100% OK</h3>
            <p class="mt-1 text-xs text-gray-500 font-medium">Postgres AWS Singapore</p>
          </div>
          <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-success-50 text-success-500 dark:bg-success-500/15">
      ...
      <!-- Superadmin Overview Tables & System Cards -->
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
          <h3 class="font-bold text-base text-gray-900 dark:text-white mb-2 flex items-center gap-2">
            <span>🏢 Portofolio Seluruh Workspace</span>
          </h3>
          <p class="text-xs text-gray-500 mb-4">Daftar tenant workspace aktif di platform MariFlow SaaS.</p>
          <div class="space-y-3">
            <div
              v-for="ws in workspaceStore.workspaces"
              :key="ws.id"
              class="flex items-center justify-between p-3 rounded-xl border border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition"
            >
              <div>
                <span class="font-bold text-sm text-gray-900 dark:text-white">{{ ws.name }}</span>
                <span class="block text-[11px] text-gray-400 font-mono">slug: {{ ws.slug }}</span>
              </div>
              <router-link to="/workspaces">
                <BaseButton variant="outline" size="sm">Kelola</BaseButton>
              </router-link>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 space-y-4">
          <h3 class="font-bold text-base text-gray-900 dark:text-white flex items-center gap-2">
            ...
            <span>Status Engine & Arsitektur Superadmin</span>
          </h3>
          <div class="space-y-2 text-xs">
            <div class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-800">
              <span class="text-gray-500">Model Role</span>
              <span class="font-bold text-gray-900 dark:text-white">Enum PostgreSQL ('superadmin', 'owner', ...)</span>
            </div>
            <div class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-800">
              <span class="text-gray-500">Isolasi Multi-Tenant</span>
              <BaseBadge variant="success" text="RLS Enabled" size="sm" />
            </div>
            <div class="flex justify-between py-2 border-b border-gray-100 dark:border-gray-800">
              <span class="text-gray-500">Region Server</span>
              <span class="font-bold text-brand-600">Singapore ap-southeast-1</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- ============================================================================== -->
    <template v-else-if="effectiveRole === 'owner'">
      <!-- Executive KPI Cards -->
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <!-- Total Projects -->
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 flex items-center justify-between">
...
const roleOptions = [
  { value: 'superadmin' as UserRole, label: '👑 Superadmin' },
  { value: 'owner' as UserRole, label: '👑 Owner (Eksekutif)' },
  { value: 'admin' as UserRole, label: '⚡ Admin (Operasional)' },
  { value: 'member' as UserRole, label: '🛠️ Member (Pribadi)' },
  { value: 'viewer' as UserRole, label: '👁️ Viewer (Read-Only)' },
]
...
const getRoleBadgeText = (role: UserRole) => {
  if (role === 'superadmin') return '👑 Superadmin Global'
...
const getRoleDescription = (role: UserRole) => {
  if (role === 'superadmin') {
    return 'Pusat kontrol master platform SaaS: visibilitas total seluruh tenant, pengguna global, dan kesehatan sistem.'
  }
  if (role === 'owner') {
